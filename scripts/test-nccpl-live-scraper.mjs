...
function parseFromKnownSections(html, options = { debug: false }) {
  ...
  if (options.debug) {
    log(`Main table found: ${selectedNormalTableHtml ? "yes" : "no"}`);
    log(`Sector table found: ${selectedSectorTableHtml ? "yes" : "no"}`);
  }
  ...
  if (options.debug) {
    log(
      `Main table rows found: ${parsedNormalTable.rows.length}; Sector table rows found: ${parsedSectorTable.rows.length}`
    );
  }
  ...
  if (options.debug) {
    log(`Extracted sessionDate: ${sessionDate ?? "none"}`);
  }
  if (!sessionDate) {
...
function logDebugSnapshot(fetchResult, html) {
  ...
  log(`Page title: ${fetchResult.title}`);
...
async function main() {
  ...
  if (args.debug) {
    log(`Number of sectors parsed: ${normalized.sectors.length}`);
  }
